...
describe("upsertChunks", () => {
  test("upserts all chunks successfully", async () => {
    ...
    const result = await upsertChunks(chunks, "http://localhost/v", "key", () => {}, mockFetch);
  ...
  test("sends correct headers and body", async () => {
    ...
    await upsertChunks(chunks, "http://localhost/v", "my-key", () => {}, mockFetch);
  ...
  test("handles empty chunk array", async () => {
    ...
    const result = await upsertChunks([], "http://localhost/v", "key", () => {}, mockFetch);
    expect(result.upserted).toBe(0);
    expect(result.errors).toBe(0);
