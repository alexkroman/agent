...
--
-- HISTORICAL, and do not edit the statements: this migration is applied, so its
-- SQL is frozen and only these comments may move.
--
-- It WAS generated by `scripts/sync-workflow-schema.mjs` and held current by a
-- gate, `pnpm check:workflow-schema`. Both are DELETED, along with the reason
-- for either: `@workflow/world-postgres` is declared by no package now, so there
-- is no upstream to re-copy from and nothing left to keep in step. The schema
-- these statements create is itself being retired —
-- `20260901010000_drop_workflow_devkit_schema.sql` renames it to
-- `workflow_retired`, and a later release drops it.
--
...
--
-- The `-- devkit-entry:` lines below were how the deleted gate knew what this
-- copy covered — read back out of every committed migration, so there was no
-- second manifest to go stale. Nothing reads them now; they stay because they
-- are the record of which upstream entries these frozen statements ARE.
