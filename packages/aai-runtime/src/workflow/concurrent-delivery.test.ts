 ...
 *
 * `workflow/_concurrent-harness.ts` closes that: one engine, one journal, every
 ...
 * arm is honestly a model of); `workflow/_laws-harness.ts` carries the reason
...
import { noteScenario, type Stats, zeroStats } from "./_reach-harness.ts";
import {
  type HookMode,
  type Leaf,
  label,
  ...
  type Program,
  runScenario,
} from "./_resume-harness.ts";
import type { Arm } from "./_schedule-harness.ts";
