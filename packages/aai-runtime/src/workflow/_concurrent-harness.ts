 ...
 * DRIVER: `workflow/_schedule-harness.ts` is the wire every journal call goes
 * through, `workflow/_laws-harness.ts` is what the result is read back as, and
 * `workflow/_reach-harness.ts` counts what the interleaving reached.
 * The bodies are `workflow/_resume-program.ts`, the same grammar the two
 ...
 * are sequential by construction: `workflow/_resume-harness.ts` takes the
 * delivery away at a step boundary, and `workflow/_rebuild-harness.ts` tears the
...
import { type Program, type Recorder, runProgram, tokensOf } from "./_resume-program.ts";
import {
  type Arm,
  createOpLog,
  deliveredTokens,
...
export type ConcurrentOptions = {
   ...
   * `workflow/_defective-journal.ts`. That is what makes the five laws
...
export type ConcurrentScenario = JournalOutcome & {
   ...
   * `workflow/journal/_log.ts` for why this is not the op log beside it.
   *
