 ...
 * defect lived. `workflow/_rebuild-harness.ts` carries the table of what each
 * model destroys.
 *
 ...
 *
 * The GRAMMAR is the arbitraries below; `workflow/_resume-harness.ts` is what
...
import { type RebuildScenario, runRebuildScenario } from "./_rebuild-harness.ts";
import {
  expectedOutput,
  fails,
  type HookMode,
