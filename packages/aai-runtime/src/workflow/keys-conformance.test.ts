 ...
 * for one package over, and `workflow/journal/conformance.test.ts` and
...
describe("the workflow-keys conformance registry", () => {
  ...
  test("the case list reaches for no implementation of its own", () => {
    ...
    const cases = FILES.filter(
      (f) => f.startsWith("keys-conformance-") && !isTest(f) && !f.includes("scenario"),
    );
 ...
 *
 * This is `workflow/journal/conformance-arms.test.ts` for the key index, and it is worth
 ...
 * which is the self-reference that cost `workflow/journal/conformance-arms.test.ts` a run.
