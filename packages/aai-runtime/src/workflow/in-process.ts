...
import { signalRunSettled } from "./run-reads.ts";
import { createMemoryStreams, type StreamStore } from "./streams.ts";
...
export function createInProcessWorkflowEngine(
  ...
  engine = withSettleSignal(
    createWorkflowEngine({
      ...
      // owes is uniqueness and the grammar `workflow/_run-id.ts` will accept.
