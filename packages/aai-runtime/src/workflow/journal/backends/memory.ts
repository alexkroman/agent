...
type Slot = {
   ...
   * per charge — `workflow/journal/_attempts.ts` carries the argument. The
...
function wakeReaches(
  ...
  // `workflow/journal/conformance-waits.ts`, which is why it is a strict compare now.
...
export function createMemoryJournal(): JournalStore {
  ...
  return {
    ...
    async claimAttempt(
      ...
      // took and answers `0`. `workflow/journal/_attempts.ts` argues it.
