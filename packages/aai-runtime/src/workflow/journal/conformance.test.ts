 ...
 *
 * The Postgres arm is `workflow/journal/conformance-postgres.scenario.test.ts`.
 *
 ...
 * (`workflow/_defective-journal.ts`'s `silentDuplicateCreate`) and freeze an
...
describe("the journal conformance registry", () => {
  ...
  test("every journal backend module in the tree is registered", () => {
    ...
    // wider now, since a backend can no longer arrive under a name it fails to
    // recognise, only in a directory it does not read.
    const modules = FILES.filter((f) => !isTest(f) && exportedFunctions(f).some(isFactory));
  ...
  test("every registered backend declares at least one arm", () => {
    ...
    // it declares is `workflow/journal/conformance-arms.test.ts`, which reads the whole
