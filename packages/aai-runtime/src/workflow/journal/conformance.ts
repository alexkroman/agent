 ...
 *   `workflow/journal/conformance.test.ts`. That fake decodes and encodes exactly what
 ...
 *   down in this file's own report and in `workflow/journal/conformance.test.ts`.
 * - **postgres** — in the SCENARIO tier
 *   (`workflow/journal/conformance-postgres.scenario.test.ts`), behind `describeWithPg`,
 ...
 *   `workflow/journal/conformance.test.ts` sweeps the tree and fails when a
 *   `workflow-journal-*.ts` factory is missing from {@link JOURNAL_BACKENDS},
 *   and `workflow/journal/conformance-arms.test.ts` fails when a declared
...
import { type JournalArm, journalRunConformance } from "./conformance-cases.ts";
...
export { type JournalArm, journalIds, keysFor, runOf } from "./conformance-cases.ts";
...
export type JournalBackend = {
   ...
   * as one beside the registry, and `workflow/journal/conformance-arms.test.ts` reads
 ...
 *
 * `module` and `factory` are what the sweep in `workflow/journal/conformance.test.ts`
 * matches the tree against, so this list is the one place a fourth backend is
 * declared; `arms` is what `workflow/journal/conformance-arms.test.ts` reads, and is the
...
export const JOURNAL_BACKENDS: readonly JournalBackend[] = [
  {
    ...
    arms: [
      {
        file: "aai-runtime/src/workflow/journal/conformance.test.ts",
        tier: "unit",
  ...
  },
  {
    ...
    arms: [
      {
        file: "aai-runtime/src/workflow/journal/conformance-postgres.scenario.test.ts",
        tier: "scenario",
