 ...
 *
 * `workflow/journal/conformance.ts` is the entry point and carries the whole argument for
 ...
 * The wait half is `workflow/journal/conformance-waits.ts`, the resume half
 * `workflow/journal/conformance-resume.ts` and the typed-JSON half
 * `workflow/journal/conformance-codec.ts`; each split is the file-length cap's doing and
...
export type JournalArm = {
   ...
   * is the platform queue's reconcile. So `workflow/journal/conformance-resume.ts`'s ten
   ...
   * `workflow/journal/conformance-resume.ts` pins the declaration against the probe in a
