 ...
 * `workflow/journal/_invariants.ts` re-derives what the log must satisfy without
 * asking the store whether it thinks so.
 *
 * ## It is NOT the op log next door, and the difference is the whole point
 *
 * `workflow/_schedule-harness.ts` also wraps every journal method and also keeps
 ...
 * `workflow/journal/conformance.ts`'s question and has its own tier.
 */
