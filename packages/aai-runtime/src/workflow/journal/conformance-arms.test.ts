 ...
 *
 * `workflow/journal/conformance.test.ts` holds the registry's own guard — every backend
 ...
 * unchanged, and `workflow/journal/conformance.test.ts`'s own registry fully green — the
...
import { JOURNAL_BACKENDS } from "./conformance.ts";
...
const CASE_MODULES = [
  "aai-runtime/src/workflow/journal/conformance-cases.ts",
  "aai-runtime/src/workflow/journal/conformance-codec.ts",
  "aai-runtime/src/workflow/journal/conformance-waits.ts",
  "aai-runtime/src/workflow/journal/conformance-resume.ts",
];
 ...
 */
const SELF = "aai-runtime/src/workflow/journal/conformance-arms.test.ts";
...
describe("the reverse direction, and the floors under both", () => {
  ...
  test("at least one arm still RUNS the resume half", () => {
    ...
    // De-duplicated by FILE, because two arms share `workflow/journal/conformance.test.ts`
    ...
    expect(running).toEqual([
      "aai-runtime/src/workflow/journal/conformance-postgres.scenario.test.ts",
      "aai-runtime/src/workflow/journal/conformance.test.ts",
    ]);
  });
