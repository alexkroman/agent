 ...
 * `workflow/journal/conformance.ts` is the arm that holds the three against each
 ...
 *
 * The reason `workflow/_laws-harness.ts` gives for the same shape: a chain
...
import { isTerminalMove } from "./_log.ts";
import { isTerminalStatus, type RunStatus, type StepEntry } from "./types.ts";
 ...
 *
 * `JSON.stringify`, which is what `workflow/_laws-harness.ts` and
 ...
 * than the store's — the same claim `workflow/_laws-harness.ts` makes as law 3
