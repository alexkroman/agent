 ...
 * the seams that file already had: `workflow/_schedule-harness.ts` is the wire,
 * `workflow/_concurrent-harness.ts` drives the deliveries, and this reads the
 * result and the op log back as claims. That test's own doc lists the five and
 * carries the argument.
 *
 * The seam against `workflow/_reach-harness.ts` is the one worth naming: a LAW
...
import type { ConcurrentScenario } from "./_concurrent-harness.ts";
import type { Scenario } from "./_resume-harness.ts";
import { expectedOutput, fails, type Program } from "./_resume-program.ts";
import { callArgs, type Ev, isGuardedTerminalMove } from "./_schedule-harness.ts";
import { isTerminalStatus, type StepEntry } from "./journal/types.ts";
