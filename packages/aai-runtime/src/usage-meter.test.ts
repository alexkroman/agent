...
/** A meter plus the snapshots it announced, which is the observable surface. */
function meterWith(limits?: { totalTokens?: number }) {
  const updates: UsageSnapshot[] = [];
  const meter = createUsageMeter({ limits, onUpdate: (snapshot) => updates.push(snapshot) });
  return { meter, updates };
}

describe("createUsageMeter", () => {
  test("accumulates across steps and announces the running total", () => {
    const { meter, updates } = meterWith();
  ...
  test("a limit is reached at or past the cap, and the reason names both numbers", () => {
    const { meter } = meterWith({ totalTokens: 500 });
