...
/**
 * Frozen authoring template: `aai-runtime:eval` epoch 8.
 *
 * Driving an agent from TEXT to measure what it did, written the way it was
 * authored at epoch 8 — a session with its two speech stages faked, the readers
 ...
 *
 * Epoch 9 added four READERS and removed nothing: `runCodeIn` and
 * `runCodeOutput` (the `run_code` arguments and results four template evals had
 * each read through a hand-written schema, the second THROWING on the executor's
 * refusal rather than letting a spec match its sentence), `expectToolBeforeSpeech`
 * (two evals had pinned the event names by hand), and `errorsIn` beside a new
 * `EvalTurn.errors` field. A reader is a pure function over values an epoch-8
 * case already held — `turn.toolCalls`, `session.events()` — so a case written
 * without one compiles unchanged, and a field ADDED to `EvalTurn` reaches no
 * case that only reads the fields it has. That is what makes this a retain
 * rather than a drop.
 *
 * Nothing here names an epoch-9 export; {@link auditLookup} reads a tool's
 * arguments through `toolArgsIn` by hand, the shape `runCodeIn` specialises,
 * and it is frozen because four evals looked like it.
 *
