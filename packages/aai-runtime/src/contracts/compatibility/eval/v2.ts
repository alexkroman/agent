...
/**
 * Frozen authoring example: `aai-runtime:eval` epoch 2.
 *
 ...
 *
 * It is `v1.ts` plus the two names epoch 2 added — `expectCalled` and
 * `lastToolResultIn` — used where a case would really reach for them, which is
 * the whole difference between the two files. Read `v1.ts` first: everything
 * this one says about SHAPE it says there.
 *
 * ## Two things about its SHAPE, both imposed rather than chosen
 *
 * **It names every one of epoch 2's sixty-nine exports.** The gate requires it
 ...
 *
 * Nothing here uses `describeTextEval`, `evalTextCredentials` or the two text
 * case types. Those are what moved the capability to epoch 3 — a TEXT agent's
 * own suite, which epoch 2 had no door to — and this file is the promise about
 * the epoch before that one.
 *
 * @module
 */
...
describeEval(agentDef, (test) => {
  ...
  test(
    ...
    async ({ session }) => {
      ...
      // The session-wide claim, as a THROW naming what was called instead:
      // `expect(names).toContain(…)` prints a list, and this prints the list
      // plus which name is missing from it.
      expectCalled(turns, "look_up_order");
      expect(turnCalling(turns, "look_up_order"), describeToolCalls(calls)).toBeDefined();
      // The NEWEST answer, for a tool a session may call more than once —
      // `toolResultIn` insists on exactly one call and is right for one turn.
      expect(lastToolResultIn(calls, "look_up_order", Order).id).toBe("W1");
...
// ─── The rest of epoch 2's surface, named so the epoch is FROZEN ─────────────
