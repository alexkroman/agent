...
/**
 * Frozen authoring example: `aai:state` epoch 17.
 *
 ...
 * was authored at epoch 17, and it must keep compiling for as long as that
 * epoch is advertised as supported.
 *
 * ## What moved, and why epoch 17 survives it
 *
 * Epoch 18 ADDED `SlotCaps` and, on `SessionSlotOptions`, the optional `caps`
 * field it types — a per-array growth cap the slot enforces after every write,
 * so a `push` to a capped list cannot grow what the model reads or what crosses
 * the wire for the length of a call. Additive: no existing signature moved and
 * nothing must be set, so a slot declared without `caps` — every slot below —
 * compiles and behaves exactly as it did.
 *
 * Nothing here names `SlotCaps` or passes `caps`, deliberately. An epoch-17
 * agent capped its lists with `pushCapped` in a wrapper of its own, and this
 * file is evidence about that surface, not a demonstration of the better way.
 * {@link checkInLabs} appends to `visits` uncapped, which is exactly the shape
 * epoch 18's option exists to replace, and it has to keep compiling.
 *
 * **The directions that WOULD break this file** are the ones a slot's own
 * contract turns on. `update` losing its synchronous draft window; `get`
 * widening from {@link DeepReadonly} back to a shallow `Readonly`, which would
 * make {@link frozenBoard}'s helpers compile against a value the runtime still
 * freezes; `projection` becoming a plain record rather than the CALLABLE that
 * renders a session which has run no tool; `SlotHolder` requiring a full
 * `ToolContext` again, which is what {@link holderFor} depends on not doing;
 * and `caps` becoming REQUIRED, which is the one direction the new field could
 * move in that would reach a file that never wrote it.
 *
