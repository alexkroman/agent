...
/**
 * Frozen authoring example: `aai:testing` epoch 28.
 *
 * A template's spec as it was authored at epoch 28 — build the context a tool
 ...
 * for as long as epoch 28 is advertised as supported.
 *
 * ## What moved, and why epoch 28 survives it
 *
 * Epoch 29 ADDED seven names: `expectDeployable`, `expectPromptBuiltinsDeclared`
 * and `commandedBuiltins` (the three starter-spec invariants six templates had
 * each written out), `runGuardrail` (a subagent's guardrail called the way the
 * runtime calls it), and `scriptedToolContext` with its two types (a context
 * with BOTH model seams scripted, which three specs had composed by hand).
 * Additive: no existing signature moved, and nothing a spec already called
 * behaves differently.
 *
 * What the additions replace is written out below, which is the whole point of
 * freezing it: {@link context} is the `stubGenerate` + `stubDelegate` +
 * `createToolContext` composition `scriptedToolContext` folds, and it has to
 * keep compiling whether or not it is ever converted. What epoch 28 DID have
 * that epoch 27 did not is the refusal pair — {@link refusedAt} reads a gate's
 * refusal through `expectDialogRefused` and an eval's serialized copy through
 * `dialogRefusalPattern`, where `v27.ts` still spells the sentence by hand.
 *
 * Nothing here names an epoch-29 export.
 *
 ...
 * epoch 28's promise too and it is frozen here; the six `install*` slot fakes are
 * frozen in `v28-slots.ts` for the same reason.
 *
 ...
 * error IS the finding, and it means epoch 28 has to be dropped with a reason.
 *
 * ## The example is two files
 *
 * The published step slots and the reporter live in `v28-slots.ts`, the same
...
/**
 * The context a tool body is handed, with the defaults epoch 28 filled. The
...
/**
 * The gate's own sentence, as an epoch-28 eval matched it: a pattern DERIVED
 * from the sentence the gate writes, so a rewording cannot strand the spec. It
 * admits the JSON escaping a serialized tool result carries.
 */
export const refusalAt = (state: string): RegExp => dialogRefusalPattern(state);

/**
 * Drive `name` before the dialog has reached its state and read the refusal
 * back — the shape every gated-tool spec had at epoch 28. `expectDialogRefused`
 * THROWS when the gate did not hold, naming where the dialog landed, which is
 * what the `isToolFailure` + `if` shape it replaced could not do.
 */
export async function refusedAt(
  ...
  return expectDialogRefused(await bind(agent)(name, undefined, context()), state).error;
}
