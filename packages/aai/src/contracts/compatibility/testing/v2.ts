 ...
 *
 * Epoch 3 is COLLATERAL, and this file is the evidence that collateral is all
 * it was. Not one helper here changed signature; what changed is the
 * declaration they are pointed AT. `AgentDef` gained `description` and, by
 * extension, the model-tuning, guardrail and observation field groups;
 * `AgentConfigSource` — what `expectDeployable` takes — followed it, and
 * `HOST_ONLY_AGENT_FIELDS` grew the two guardrail arrays. Every addition is
 * optional, so a spec that hands `deployedAgent` an agent written before any
 * of them existed still compiles, and so does one that reads
 * `expectDeployable`'s answer field by field.
 *
 * The `ToolDef` a tool spec reaches through `toolOf` and `runTool` gained an
 * optional `onError` in the same change, and `Message` — which
 * `createToolContext` seeds as `ctx.messages` — gained `toolName` and
 * `toolCallId`. Both are pinned below: `historyContext` passes a `messages`
 * override built from two-key literals, which a required field would have
 * broken.
 *
 ...
 *
 * **It names every one of epoch 2's 93 exports.** The gate requires it
