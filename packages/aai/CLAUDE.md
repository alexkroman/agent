...
**Open vocabularies spell their literals INLINE: `"a" | "b" | (string & {})`** —
...
by `agentConfigWarnings` at build. No closed `Known*` half is PUBLISHED beside
one: a closed union an author can import changes what it accepts on every
refresh, so the probe could never prove it compatible. Code needing the closed
half derives it (`KnownLiterals<T>`, `sdk/is-known.ts`) — `KnownLlmProvider` and
