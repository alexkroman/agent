...
| AssemblyAI `min_turn_silence` | 3000 (`DEFAULT_MIN_TURN_SILENCE_MS`) | `host/providers/stt/assemblyai.ts` | End-of-turn silence before the service commits a `final`. Endpointing lives in the STT provider — the pipeline transport commits a turn on every final — so this is what keeps a mid-utterance pause from splitting one request across turns. Raised 1500 -> 2000 -> 3000, each step on the same measurement: at 2000, Full-Duplex-Bench v3's hesitation recording STILL split — the agent answered the fragment with "Sure. What", was barged in on when the caller resumed, and the real reply landed 15s later. tau2-bench shows the same failure independently as its `SPLIT` mis-hearing class. Note 3000 now EXCEEDS `DEFAULT_FALSE_INTERRUPTION_TIMEOUT_MS` (2000), which is safe only because a fired recovery window whose utterance is still open DEFERS the resume; the benchmark's own breakdown localizes it to *silence* rather than word accuracy (self-corrections and false starts passed 100%, hesitations 33%, pauses 57%). Override via `assemblyAIStt({ minTurnSilenceMs })`. |
