...
`OpenAIS2sVoice`, `AssemblyAIReasoningEffort`. The literals autocomplete; an
unknown value compiles, is accepted by `AgentConfigSchema`, and is WARNED about
by `agentConfigWarnings` at build. **Only the inline form makes a changed list
compatible.** This section used to say a regenerated known list was a revision
whatever the spelling, and it was false for `Known… | (string & {})`: the
exported CLOSED `Known…` half is a type of its own, and a union that grows is
not assignable back to the one it grew from, so every added name was a break of
that export. Inline, the probe proves the change mutually assignable. So no
closed half is PUBLISHED: a reader that must be TOTAL over the shipped names
derives it (`KnownLiterals<T>` in `sdk/is-known.ts`, `keyof typeof
VOICE_PRESETS`, `(typeof TELEPHONY_CARRIERS)[number]`) — `KnownLlmProvider` and
`KnownGatewayModel` are on `/host-internal`.
...
`_llm-registry.ts` plus its literal in `LlmProviderName` and `KNOWN_LLM_PROVIDERS`
(the registry's `satisfies Record<KnownLlmProvider, …>` holds them together).
...
as the non-empty tuple `z.enum` takes; `ttsVoiceInfo(voice)` is the lookup by an
open id (own keys only, `undefined` for an unlisted voice).
