...
- **Pipeline mode**: user speaks → browser captures PCM → WebSocket →
  ...
  `resumeFalseInterruption`). `preemptiveGeneration` (ON by default) opens a
...
| `resumeFalseInterruption` | `true` | `pipeline-transport-options.ts` | Pipeline only: a partial-triggered barge-in that never commits a user turn (STT noise) resumes the interrupted reply via a synthetic continuation turn. `false` disables. **It is a boolean because the WAIT cannot be an author knob.** The resume fires when the transcript stream goes quiet with no committed final — the speaking edge's idle watchdog, `DEFAULT_SPEECH_IDLE_TIMEOUT_MS` (3500, internal) — and nothing shorter is safe: this was a `falseInterruptionTimeoutMs: number` defaulting to 2000, measured from roughly the same instant as the STT's `min_turn_silence`, so EVERY genuine barge-in raced its own resume and the resume won often enough to be the common case. Each one cost a billed LLM turn, put "the user did not actually say anything" in history directly ahead of the real user turn, and (TTS time-to-first-audio ~350ms) made the caller hear the agent continue the reply they had just interrupted. The floor on the deadline is the STT's endpointing plus final-emission latency, which the transport cannot see — it receives an already-resolved `SttOpener` — and the ceiling is patience (at 5000 a reply cut by noise resumed almost six seconds later, which reads as a dropped call), so there is no useful range to expose. The old number never governed anything anyway: a probe at `falseInterruptionTimeoutMs: 3` resumed at ~3500ms. A mid-turn cut resumes from the `[interrupted]` history marker (`DEFAULT_FALSE_INTERRUPTION_PROMPT`) only when no cut point is known; otherwise, and always for a cut during the client playback tail, the prompt quotes the estimated last-heard words (`buildTailResumePrompt`) — measured, resuming from the marker instead repeated 60%+ of the words in 10% of consecutive agent utterances, because TTS runs behind the text. That anchor is now the SAME cursor history is truncated with (`pipeline-heard.ts`), so it can never name words the record denies, and it is word-accurate wherever the TTS provider reports timings. A tail cut with less than `TAIL_RESUME_MIN_UNHEARD_MS` unheard arms nothing. |
| `preemptiveGeneration` | `true` | `pipeline-speculation.ts` | Pipeline only, **ON by decision and STILL UNMEASURED here** — the default was flipped without either instrument below existing, so what makes it defensible is the two structural guardrails rather than evidence of a saving: the worst case is a discarded stream (one extra billed LLM request for that utterance) and a turn that runs byte-identically to the `false` path. Starts the reply from a high-confidence STT INTERIM (`SttTurnMeta.endOfTurnConfidence` ≥ `PREEMPTIVE_CONFIDENCE_THRESHOLD`, 0.9, a judgement call) and, when the committed final says the same thing, ADOPTS that already-running stream — a head start of real generation, not a cache lookup. **Two guardrails, both structural rather than flags.** (1) No speculative speech: `createStreamPartHandler` is the only path from a stream part to `sendTtsText` and it is built only inside `consumeLlmStream`, while the speculative drainer's only sink is an array — adoption IS the act of connecting the tape to the handler. (2) No speculative tool execution: the tool set comes from `toDeclaredTools`, which omits the `execute` property, so the SDK cannot continue past a tool call; a speculation that reaches one is discarded WHOLE, preamble included (adopting the preamble alone would need an assistant prefill, which changes the request shape). **Match rule**: `normalizeUtterance(final) === normalizeUtterance(partial)` — lowercase, punctuation→space — because a partial and its final routinely differ in formatting alone (`turn_is_formatted` in the STT adapter's turn trace; a provider property, not a measurement). An extension, a truncation or a revision all DISCARD, and the extension case is deliberate: answering half an utterance is the failure `DEFAULT_MIN_TURN_SILENCE_MS` records at 5.7x reward. **Sawtooth rules** (the confidence trace on `SttTurnMeta.endOfTurnConfidence` dictated all three): a partial whose normalized text differs aborts the live speculation at once, an identical text at rising confidence never re-fires, and at most `MAX_PREEMPTIVE_SPECULATIONS_PER_UTTERANCE` (2) per utterance. Also gated on the transport being idle, on the history revision not having moved, and inert unless `toolChoice` is `"auto"`/`"none"`. **Its reach is BOUNDED BY a measurement, not shown by one**: 28-33% of replies called a tool at all across 815 replies in two tau2 retail runs (see `DEFAULT_MAX_STEPS`), so at most the other 67-72% can ever be accelerated, and a tool-calling agent pays one wasted request per speculating utterance for nothing. **The two measurements are still owed, and they are now what would justify turning it back OFF**: the `headStartMs`/adoption-rate log (`Pipeline speculation adopted` at info, `Pipeline speculation discarded` at debug with a reason) over a caller-audio replay run — a rare adoption or a ~0 head start makes this pure cost — AND a tau2-bench retail run at the same 25 tasks and seed showing no reward regression. (The in-repo `scripts/voice-replay/` harness that produced those logs has been removed.) The shipped value is pinned twice: at the resolver (`pipeline-transport-options.test.ts`) and end-to-end through the transport (`pipeline-preemption.test.ts`), so a flip in either direction is a deliberate edit. A speculation must never call `emitError` — it has no reply the client knows about. |
