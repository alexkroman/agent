...
| `@alexkroman1/aai/workflow-api` | `sdk/workflow-api-barrel.ts` → 4 modules | The CLIENT of everything a deployed agent answers — the surface written for a caller OUTSIDE the agent (a page, a script, a cron job). **`createAgentClient` is the one to reach for**: one object over `config()` (the front door) and every workflow route. `createWorkflowApiClient` is the narrower factory a page uses. `WorkflowApi.follow`/`.followOutput` are `for await` iterators honouring the two SSE continuation rules `watch`/`streamOutput` leave to a caller; `readEventStream` is the parser under them, public so aai-ui carries no second copy. The graph is zod-FREE (a workflow app's page bundles it) — hence `sdk/client-config-path.ts`. Each module's doc carries why |
