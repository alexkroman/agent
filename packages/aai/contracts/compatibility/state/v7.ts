...
/**
 * Frozen authoring example: `aai:state` epoch 7.
 *
 ...
 * reaches that type through `ToolContext` (`slot.tool` hands one to every body),
 * so its hash moved while nothing an author writes here did. `aai:workflow`
 * epoch 7 is where the addition is demonstrated.
 *
