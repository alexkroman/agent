 ...
 *
 * **Epoch 8 is a RIPPLE, not a change to this capability.** `WorkflowClient`
 * gained `signal` — answer a run parked on a hook — and this capability's report
 * reaches that type through `ToolContext`, so its hash moved while nothing an
 * author writes here did. `aai:workflow` epoch 7 is where the addition is
 * demonstrated.
 *
 * So this file is epoch 7's example, re-frozen: what it proves is exactly
