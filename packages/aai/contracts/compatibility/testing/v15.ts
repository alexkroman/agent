...
/**
 * Frozen authoring example: `aai:testing` epoch 15.
 *
 * **Moved for a TRANSITIVE reason, as epochs 13 and 14 did**, and by the same
 * additive protocol change that moved `aai:agent` to 13: `SessionEvent` gained
 * `history.restored`. Nothing on `/testing` was added, removed or renamed — the
 * export list is identical to epoch 14's — but `createToolContext`'s `ctx.sent`
