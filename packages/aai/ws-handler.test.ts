...
describe("wireSessionSocket", () => {
  ...
  test("audio chunks are dropped when bufferedAmount exceeds high-water mark", () => {
    let capturedClient!: ClientSink;
    const ws = new MockWebSocket("ws://test");
    ws.readyState = MockWebSocket.OPEN;
    ...
    // Clear config message from sent
    ws.sent.length = 0;

    // Simulate backpressure: 2 MB buffered
    ws.bufferedAmount = 2 * 1024 * 1024;

    capturedClient.playAudioChunk(new Uint8Array([1, 2, 3]));
    capturedClient.playAudioChunk(new Uint8Array([4, 5, 6]));

    // Audio chunks should be dropped, not sent
    expect(ws.sent).toHaveLength(0);
  });

  test("text events are queued when backpressured and drained when pressure eases", () => {
    let capturedClient!: ClientSink;
    const ws = new MockWebSocket("ws://test");
    ws.readyState = MockWebSocket.OPEN;
    ...
    // Text event should be queued, not sent
    expect(ws.sent).toHaveLength(0);

    // Ease backpressure
    ws.bufferedAmount = 0;
    // Next send triggers drain of the queued text
    capturedClient.event({ type: "speech_stopped" });

    // Both the queued event and the new one should be sent
    const sentStrings = ws.sent.filter((d): d is string => typeof d === "string");
    expect(sentStrings.some((s) => s.includes('"speech_started"'))).toBe(true);
    expect(sentStrings.some((s) => s.includes('"speech_stopped"'))).toBe(true);
  });

  test("text queue is bounded — oldest events are dropped when full", () => {
    let capturedClient!: ClientSink;
    const ws = new MockWebSocket("ws://test");
    ws.readyState = MockWebSocket.OPEN;
    ...
    // Queue 65 events (max is 64), first should be dropped
    for (let i = 0; i < 65; i++) {
      capturedClient.event({ type: "speech_started" });
    }

    // Ease pressure and trigger drain
    ws.bufferedAmount = 0;
    capturedClient.event({ type: "speech_started" });

    // 64 queued + 1 new = 65 sent
    const sentStrings = ws.sent.filter((d): d is string => typeof d === "string");
    expect(sentStrings).toHaveLength(65);
  });

  test("sends normally when bufferedAmount is below threshold", () => {
    let capturedClient!: ClientSink;
    const ws = new MockWebSocket("ws://test");
    ws.readyState = MockWebSocket.OPEN;
    ...
    ws.sent.length = 0;
    ws.bufferedAmount = 100; // Well below 1 MB threshold
