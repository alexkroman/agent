...
describe("createRuntime", () => {
  ...
  test("pipeline relay mode does NOT emit an observability tool_call frame", async () => {
    ...
    const exec = createRuntime({
      ...
      // Presence of executeTool + toolSchemas + onToolResult puts the runtime in
      // relay mode (isRelay = Boolean(opts.onToolResult)).
    ...
    const toolCallEmits = (client.event as ReturnType<typeof vi.fn>).mock.calls.filter(
      ([e]) => (e as { type: string }).type === "tool_call",
    );
    expect(toolCallEmits).toHaveLength(0);
  });

  test("pipeline in-process mode DOES emit one observability tool_call frame", async () => {
    // Counterpart to the relay case: a deployed pipeline agent executes tools
    // in-process, so the observability emit is the ONLY `tool_call` signal to
    // the client and must be preserved (exactly once).
    const stt = createFakeSttProvider();
    const tts = createFakeTtsProvider();
    const client = makeClientSink();
    const exec = createRuntime({
      agent: makeAgent({
        tools: { lookup: { description: "Look up", execute: () => "ok" } },
      }),
      env: {},
      stt,
      tts,
    ...
    expect(toolCallEmits).toHaveLength(1);
    expect(toolCallEmits[0]?.[0]).toMatchObject({ type: "tool_call", toolCallId: "tc-1" });
  });
