...
export function createRuntime(opts: RuntimeOptions): Runtime {
  ...
  // The AssemblyAI Voice Agent API accepts exactly one sample rate and honours
  // no declaration to the contrary, so its rates are pinned rather than
  // negotiated — including over a host-mode client's explicit request. Pinned
  // BEFORE the ready config is built, because that frame is what tells the
  // client what to capture and play: the whole bug being fixed here is the two
  // numbers disagreeing.
