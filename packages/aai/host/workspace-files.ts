 ...
 * (regenerated by install, noise in the studio editor).
 *
 * Applied by the CLI's push, NOT by the guest's walk — the guest's walk also
 * backs the coding agent's `list_files`/`grep`, and hiding a `.env` the agent
 * itself wrote would make it invisible to the tool that needs to read it.
 */
