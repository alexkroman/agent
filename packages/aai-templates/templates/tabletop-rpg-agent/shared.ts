...
export const gameSlot = sessionSlot("game", () => structuredClone(DEFAULT_STATE), {
  ...
  // reads. `emergency-dispatch-agent`'s board is the same pattern one template over.
 ...
 * holds the campaign, which is the same split `travel-concierge-agent` makes between
 ...
 *
 * Generic over the clock's own type — as `emergency-dispatch-agent`'s `findIncident` is —
