...
import { z } from "zod";
import type { Disposition, GameState, KV } from "../shared.ts";
import { getGameState, MAX_SESSION_LOG, nextNpcId, saveGameState } from "../shared.ts";

const updateStateParams = z.object({
...
type UpdateStateArgs = z.infer<typeof updateStateParams>;

function applyResources(state: GameState, args: UpdateStateArgs): void {
...
function applyLocation(state: GameState, args: UpdateStateArgs): void {
...
function bondForDisposition(disposition: string | undefined): number {
  if (disposition === "friendly") return 1;
  if (disposition === "loyal") return 2;
  return 0;
}

function applyAddNpc(state: GameState, args: UpdateStateArgs): void {
  if (!args.addNpcName) return;
  ...
  state.npcs.push({
    ...
    bond: bondForDisposition(args.addNpcDisposition),
...
function applyUpdateNpc(state: GameState, args: UpdateStateArgs): void {
  if (!args.updateNpcId) return;
...
function applyNpcChanges(state: GameState, args: UpdateStateArgs): void {
  applyAddNpc(state, args);
  applyUpdateNpc(state, args);
}

function applyClockChanges(state: GameState, args: UpdateStateArgs): void {
...
function applyStoryArc(state: GameState, args: UpdateStateArgs): void {
...
function applySessionLog(state: GameState, args: UpdateStateArgs): void {
  if (args.logEntry) {
...
function updateCrisisState(state: GameState): void {
...
export const updateState = tool({
  ...
  async execute(args, ctx: { kv: KV; send: (event: string, data: unknown) => void }) {
    ...
    applyResources(state, args);
    applyLocation(state, args);
    applyNpcChanges(state, args);
    applyClockChanges(state, args);
    applyStoryArc(state, args);
    applySessionLog(state, args);
    updateCrisisState(state);
