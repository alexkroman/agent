 ...
 * one `retail-orders-agent` draws between `shared.ts` and `store.ts` and it is worth keeping
 ...
 * the dialog. Mixing them is the mistake `emergency-dispatch-agent`'s guide entry is
 ...
 * tool happened to write last; see `emergency-dispatch-agent` for the worked version of
 * that.
 */
