 ...
 *
 * ## What makes this a different KIND of agent from `research-handoff-agent`
 *
 * `research-handoff-agent` is a voice agent that HANDS OFF to a workflow — a caller is on
 ...
 * rather than `ctx.env`; see `workflows/digest.ts` and `research-handoff-agent`'s module
