...
const SEVERITY_WEIGHTS: Record<Severity, number> = {
  critical: 100, urgent: 70, moderate: 40, minor: 10,
};

const TYPE_MULTIPLIERS: Record<IncidentType, number> = {
  medical: 1.2, fire: 1.3, hazmat: 1.5, traffic: 1.0, crime: 1.1,
  natural_disaster: 1.8, utility: 0.8, other: 0.7,
};

function calculateTriageScore(
  severity: Severity, type: IncidentType, casualties: number, hazards: number,
): number {
...
function getApplicableProtocols(type: IncidentType, severity: Severity): Protocol[] {
  const severityRank: Record<Severity, number> = { critical: 4, urgent: 3, moderate: 2, minor: 1 };
...
function recommendResources(type: IncidentType, severity: Severity, state: DispatchState): Resource[] {
  const needed: Resource["type"][] = [];
  const baseNeeds: Record<IncidentType, Resource["type"][]> = {
    medical: ["ambulance"], fire: ["fire_engine", "ambulance"],
    hazmat: ["hazmat_team", "fire_engine", "ambulance"],
    traffic: ["police", "ambulance", "fire_engine"], crime: ["police"],
    natural_disaster: ["fire_engine", "ambulance", "police"],
    utility: ["fire_engine"], other: [],
  };
  ...
  if (severity === "urgent" && type === "fire") needed.push("fire_engine");
  ...
  for (const needType of needed) {
    ...
    if (available) { recommended.push(available); usedIds.add(available.id); }
  }
  return recommended;
}
...
async function saveState(ctx: { kv: Kv; state: DispatchState }): Promise<void> {
...
export default {
  ...
  tools: {
    incident_create: {
      ...
      parameters: {
        ...
        properties: {
          ...
          hazards: { type: "array", items: { type: "string" }, description: "Known hazards: fire, chemical, electrical, structural, weapons" },
        },
        required: ["location", "description"],
      },
      execute: async (args: Record<string, unknown>, ctx: ToolContext<DispatchState>) => {
        const location = args.location as string;
        const description = args.description as string;
        const callerName = args.callerName as string | undefined;
        const callerPhone = args.callerPhone as string | undefined;
        const estimatedCasualties = args.estimatedCasualties as number | undefined;
        const hazards = args.hazards as string[] | undefined;
        ...
        const triageScore = calculateTriageScore(recSeverity, recType, estimatedCasualties || 0, hazards?.length || 0);

        const incident: Incident = {
          id, type: recType, severity: recSeverity, status: "incoming",
          location, description, callerName: callerName || "Unknown",
          callerPhone: callerPhone || "Unknown", triageScore,
          assignedResources: [],
          timeline: [{ time: now(), event: `Incident created: ${description}` }],
          notes: [], createdAt: now(), updatedAt: now(), escalationLevel: 0,
          protocolsActivated: [],
          casualties: { confirmed: 0, estimated: estimatedCasualties || 0, treated: 0 },
          hazards: hazards || [],
        };
        ...
        return {
          ...
          recommendedResources: recommended.map((r) => ({
            callsign: r.callsign, type: r.type, capabilities: r.capabilities,
          })),
    ...
    incident_triage: {
      description: "Triage an incident — confirm or override severity, type, hazards, and casualty count.",
      parameters: {
        type: "object" as const,
        properties: {
          incidentId: { type: "string", description: "The incident ID" },
          severity: { type: "string", enum: ["critical", "urgent", "moderate", "minor"], description: "Confirmed severity after triage" },
          type: { type: "string", enum: ["medical", "fire", "hazmat", "traffic", "crime", "natural_disaster", "utility", "other"], description: "Confirmed incident type" },
          additionalHazards: { type: "array", items: { type: "string" }, description: "Any additional hazards identified" },
      ...
      execute: async (args: Record<string, unknown>, ctx: ToolContext<DispatchState>) => {
        ...
        const type = args.type as IncidentType | undefined;
        const additionalHazards = args.additionalHazards as string[] | undefined;
        const casualtyUpdate = args.casualtyUpdate as number | undefined;
        ...
        if (casualtyUpdate !== undefined) inc.casualties.estimated = casualtyUpdate;
        ...
        inc.status = "triaged";
        inc.updatedAt = now();
        inc.timeline.push({ time: now(), event: `Triaged: ${inc.severity} ${inc.type}, score ${inc.triageScore}` });
        ...
        return {
          incidentId, severity: inc.severity, type: inc.type,
          triageScore: inc.triageScore, hazards: inc.hazards,
          estimatedCasualties: inc.casualties.estimated,
          protocols: protocols.map((p) => ({ name: p.name, steps: p.steps, requiredResources: p.requiredResources })),
    ...
    incident_update_status: {
      ...
      parameters: {
        ...
        properties: {
          incidentId: { type: "string", description: "The incident ID" },
          status: { type: "string", enum: ["en_route", "on_scene", "resolved", "escalated"], description: "New status" },
      ...
      execute: async (args: Record<string, unknown>, ctx: ToolContext<DispatchState>) => {
        ...
        const notes = args.notes as string | undefined;
        const casualtyUpdate = args.casualtyUpdate as { confirmed?: number; treated?: number } | undefined;
        ...
        inc.status = status;
        inc.updatedAt = now();
        inc.timeline.push({ time: now(), event: `Status → ${status}${notes ? `: ${notes}` : ""}` });
        if (notes) inc.notes.push(notes);

        if (casualtyUpdate) {
          if (casualtyUpdate.confirmed !== undefined) inc.casualties.confirmed = casualtyUpdate.confirmed;
          if (casualtyUpdate.treated !== undefined) inc.casualties.treated = casualtyUpdate.treated;
        }
        ...
        if (status === "resolved") {
          for (const rId of inc.assignedResources) {
            ...
            if (r) {
              ...
              r.eta = null;
              setTimeout(() => { r.status = "available"; }, 2000);
            }
          }
    ...
    incident_escalate: {
      ...
      parameters: {
        ...
        properties: {
          ...
          requestMutualAid: { type: "boolean", description: "Whether to request mutual aid from neighboring jurisdictions" },
          newSeverity: { type: "string", enum: ["critical", "urgent"], description: "Escalated severity level" },
        },
      ...
      execute: async (args: Record<string, unknown>, ctx: ToolContext<DispatchState>) => {
        ...
        const requestMutualAid = args.requestMutualAid as boolean | undefined;
        const newSeverity = args.newSeverity as Severity | undefined;
        ...
        inc.updatedAt = now();
        inc.timeline.push({ time: now(), event: `ESCALATED (Level ${inc.escalationLevel}): ${reason}` });
        ...
        if (requestMutualAid) {
          ...
          state.resources.push(
            { id: `MA-${Date.now()}-1`, type: "ambulance", callsign: "Mutual-Aid-Medic", status: "available", assignedIncident: null, eta: null, capabilities: ["als"] },
            { id: `MA-${Date.now()}-2`, type: "fire_engine", callsign: "Mutual-Aid-Engine", status: "available", assignedIncident: null, eta: null, capabilities: ["structural"] },
          );
        }
        ...
        const additionalResources = recommendResources(inc.type, inc.severity, state)
          .filter((r) => !inc.assignedResources.includes(r.id));

        return {
          incidentId, escalationLevel: inc.escalationLevel,
          newSeverity: inc.severity, newTriageScore: inc.triageScore,
    ...
    incident_get: {
      ...
      execute: (args: Record<string, unknown>, ctx: ToolContext<DispatchState>) => {
        const incidentId = args.incidentId as string;
        const state = ctx.state;
        const inc = state.incidents[incidentId];
        if (!inc) return { error: `Incident ${incidentId} not found` };

        const assignedResourceDetails = inc.assignedResources.map((rId) => {
          const r = state.resources.find((r) => r.id === rId);
        ...
        return {
          ...inc, ageMinutes, assignedResourceDetails,
    ...
    incident_add_note: {
      ...
      execute: async (args: Record<string, unknown>, ctx: ToolContext<DispatchState>) => {
        ...
        return { incidentId, noteAdded: entry, totalNotes: inc.notes.length };
      },
    },

    resources_dispatch: {
      description: "Dispatch units to an incident. Can auto-dispatch recommended resources or manually specify callsigns.",
      parameters: {
        type: "object" as const,
        properties: {
          incidentId: { type: "string", description: "The incident ID" },
          callsigns: { type: "array", items: { type: "string" }, description: "Resource callsigns to dispatch. Use 'auto' for system-recommended resources." },
          autoDispatch: { type: "boolean", description: "If true, automatically dispatch recommended resources" },
          priority: { type: "string", enum: ["routine", "priority", "emergency"], description: "Dispatch priority — affects simulated ETA" },
        },
        required: ["incidentId"],
      },
      execute: async (args: Record<string, unknown>, ctx: ToolContext<DispatchState>) => {
        const incidentId = args.incidentId as string;
        const callsigns = args.callsigns as string[] | undefined;
        const autoDispatch = args.autoDispatch as boolean | undefined;
        ...
        if (autoDispatch) {
          ...
          for (const cs of callsigns) {
            ...
            if (!r) { failed.push({ callsign: cs, reason: "Not found" }); continue; }
            if (r.status !== "available") { failed.push({ callsign: cs, reason: `Currently ${r.status}` }); continue; }
        ...
        if (dispatched.length > 0) { inc.status = "dispatched"; inc.updatedAt = now(); }
        ...
        return {
          incidentId, dispatched,
          failed: failed.length > 0 ? failed : undefined,
          totalAssignedToIncident: inc.assignedResources.length,
          remainingAvailableResources: availableCount,
          systemAlertLevel: state.alertLevel,
          capacityWarning: availableCount <= 3 ? "WARNING: Resource capacity critically low. Consider mutual aid." : undefined,
        };
      },
    },
    ...
    resources_get_available: {
      ...
      parameters: {
        type: "object" as const,
        properties: {
          type: { type: "string", enum: ["ambulance", "fire_engine", "police", "hazmat_team", "helicopter", "k9_unit", "swat", "ems_supervisor", "all"], description: "Filter by resource type, or 'all'" },
        },
        required: [],
      },
      execute: (args: Record<string, unknown>, ctx: ToolContext<DispatchState>) => {
        ...
        return {
          resources: resources.map((r) => ({
            callsign: r.callsign, type: r.type, status: r.status,
            assignedIncident: r.assignedIncident, eta: r.eta, capabilities: r.capabilities,
          })),
          summary: {
    ...
    resources_update_status: {
      ...
      parameters: {
        ...
        properties: {
          ...
          status: { type: "string", enum: ["available", "dispatched", "en_route", "on_scene", "returning"], description: "New status" },
          notes: { type: "string", description: "Status notes" },
        },
        required: ["callsign", "status"],
      },
      execute: async (args: Record<string, unknown>, ctx: ToolContext<DispatchState>) => {
        const callsign = args.callsign as string;
        const status = args.status as Resource["status"];
        const notes = args.notes as string | undefined;

        const state = ctx.state;
        const resource = state.resources.find((r) => r.callsign.toLowerCase() === callsign.toLowerCase());
        if (!resource) return { error: `Resource ${callsign} not found` };

        const previousStatus = resource.status;
        resource.status = status;

        if (status === "available") { resource.assignedIncident = null; resource.eta = null; }
        ...
        return {
          callsign: resource.callsign, previousStatus, newStatus: status,
          assignedIncident: resource.assignedIncident, systemAlertLevel: state.alertLevel,
        };
      },
    },

    ops_dashboard: {
      description: "Get the full operational dashboard: alert level, resource utilization, active incidents, and available resources.",
      execute: (_args: Record<string, unknown>, ctx: ToolContext<DispatchState>) => {
        const state = ctx.state;
    ...
    ops_protocols: {
      ...
      parameters: {
        type: "object" as const,
        properties: {
          incidentType: { type: "string", enum: ["medical", "fire", "hazmat", "traffic", "crime", "natural_disaster", "utility", "other"], description: "Type of incident" },
          severity: { type: "string", enum: ["critical", "urgent", "moderate", "minor"], description: "Severity level" },
        },
      ...
      execute: (args: Record<string, unknown>) => {
        ...
        if (protocols.length === 0) {
          return { message: "No specific protocols for this combination. Use standard operating procedures.", protocols: [] };
        }
        return {
          protocols: protocols.map((p) => ({ name: p.name, steps: p.steps, requiredResources: p.requiredResources })),
        };
    ...
    ops_run_scenario: {
      ...
      parameters: {
        type: "object" as const,
        properties: {
          scenario: { type: "string", enum: ["mass_casualty", "multi_alarm_fire", "active_shooter", "natural_disaster", "highway_pileup"], description: "Scenario type to simulate" },
        },
        required: ["scenario"],
      },
      execute: async (args: Record<string, unknown>, ctx: ToolContext<DispatchState>) => {
        ...
        for (const incDef of s.incidents) {
          state.incidentCounter++;
          const id = `INC-${String(state.incidentCounter).padStart(4, "0")}`;
          const fullInc: Incident = {
            id, type: incDef.type || "other", severity: incDef.severity || "moderate",
            status: "incoming", location: incDef.location || "Unknown",
            description: incDef.description || "", callerName: "Scenario", callerPhone: "N/A",
            triageScore: calculateTriageScore(
              (incDef.severity || "moderate") as Severity,
              (incDef.type || "other") as IncidentType, 0, 0,
            ),
            assignedResources: [],
            timeline: [{ time: now(), event: `SCENARIO: ${incDef.description}` }],
            notes: [], createdAt: now(), updatedAt: now(), escalationLevel: 0,
            protocolsActivated: [],
        ...
        return {
          scenario, narrative: s.narrative, incidentsCreated: created,
          systemAlertLevel: state.alertLevel,
...
} satisfies AgentTools<DispatchState>;
