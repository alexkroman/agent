...
describe("the scope decides what a deploy carries", () => {
  ...
  test("a shipped file triggers and a test file does not", () => {
    const shipped = [
      ...
      "packages/aai-guest/package-lock.json",
    ];
    const inert = [
      "packages/aai-server/sandbox.test.ts",
      "packages/aai-server/_pg-test-utils.ts",
      "packages/aai-server/test-utils.ts",
  ...
  test("triggeringFiles groups by package and ignores everything else", () => {
    const grouped = scope?.triggeringFiles([
      "packages/aai-server/sandbox.ts",
      "packages/aai-server/sandbox.test.ts",
      "packages/aai-guest/harness.ts",
      // Not deploy-carried: published to npm, so it ships without a deploy.
      "packages/aai/sdk/agent.ts",
      "AGENTS.md",
      ".github/workflows/ship.yml",
    ]);
    expect([...(grouped?.keys() ?? [])].sort()).toEqual(["aai-guest", "aai-server"]);
    expect(grouped?.get("aai-server")).toEqual(["packages/aai-server/sandbox.ts"]);
  });
