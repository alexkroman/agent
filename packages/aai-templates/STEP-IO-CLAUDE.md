...
- **`stepGenerate`** — `research-handoff-agent` and `link-digest-workflow` had
  each hand-rolled the same forty lines and had already diverged on two of them
  (the empty-completion case, and which statuses are worth a retry).
- **`stepGenerateJson`** — the same call for a stage whose reply is a SHAPE,
  with the fence stripper, the parse, the non-object rejection and the Standard
  Schema validation each re-derived per template, the stripper already DIVERGED
  (one trimmed, one did not). Taking a schema is the point rather than a
  convenience: the predecessor was `askJson<Action>()`, a value the compiler
  believed and nothing checked, so a model answering with a plausible
  neighbouring shape flowed into the step's logic as if it had obeyed.
...
- **`toStepError(response, message)`** — the three-way call
  `transcription-workflow` and `link-digest-workflow` had hand-written
  identically. Note the third outcome is not "the engine's backoff": a bare
  `RetryableError` retries in ONE SECOND, which is that class's own default
  (`DEFAULT_RETRY_DELAY_MS`), so a fan-out that all 429s together all asks again
  a second later. Passing the far side's `Retry-After` is what drains it.
- **`toStepError` reads `StepGenerateError.retryAfter`, which THREE of the four
  templates did not.** `research-handoff-agent`, `link-digest-workflow` and
  `transcription-workflow` re-threw the error unchanged, so a rate-limited model
  call fell back to the default with the gateway's own number sitting unread on
  it. `document-redline-workflow` is the exception and worked the extra line out
  independently — which is the argument for extracting rather than a reason not
  to: the fourth author to meet a problem should not have to be the first to get
  it right.
- **`throwFatalStepError` is for the `catch` block specifically**, and the
  ...
  it. Taking the cause as an ARGUMENT is what fixes that. A
  `throw new FatalError(…)` that is NOT in a catch block stays exactly as it was
  — `link-digest-workflow`'s no-readable-text case is the worked example, and
  says so in place.
...
(`@alexkroman1/aai/testing`), which `research-handoff-agent` and
`link-digest-workflow` had each written: record the call, answer
`{choices:[{message:{content}}]}`, switch on a status. It records the `prompt`
and `system` separately, which is what the hand-rolled `promptOf(calls, n)`
reach into `body.messages[n].content` was for.
...
**`link-digest-workflow` is the same mechanism at its smallest, and it is the
FRONT DOOR that separates both of these from `research-handoff-agent`.** That
one is a voice agent that HANDS OFF to a run (a caller is on the line, so a tool
starts one and answers the turn); `link-digest-workflow` and
...
is a readability implementation and a dependency; what it MUST do is drop
`<script>` and `<style>` CONTENT rather than just their tags, because stripping
tags alone leaves a page's JavaScript in the prompt, which is both expensive and
a way to smuggle instructions past the reader. `summarize` then asks for JSON
and validates the shape, and a reply that ignored the format throws PLAINLY
where a 401 is fatal — a model may well obey on the next attempt.
...
`link-digest-workflow`'s spec asserts the DECLARATION as well as those steps,
and the declaration half is what carries the template's shape: the `page` field,
the workflow's NAME (the page starts a run by that string, so a rename is a
runtime 400 rather than a compile error), the input schema (both the call-site
validation and the JSON Schema `GET /workflows` serves), and `requiredEnv` —
which is load-bearing here in a way it is not for a voice agent, since a
workflow app declares no providers and so nothing else in its config names a
credential.
...
**Every tool-declaring template is files now, and the param is GONE.** For a
while six were not — `medication-safety-agent`, `text-adventure-agent`,
`entertainment-picks-agent`, `meeting-recap-agent`, `research-handoff-agent` and
`embedded-assets` (since removed as a near-duplicate of
`technical-support-agent`) declared theirs inline, and this guide's own
measurement missed them because it counted only the templates that already had a
`tools/` directory. That is what made the rule conventional: `agent({ tools })`
still worked, so "a tool is a file" was true of seven templates and of nothing
enforcing it. `tools` is now the `InlineToolsMisuse` message on the parameter
shape (a compile error naming the file to create) AND a throw inside `agent()` —
the second half is not belt-and-braces, it is the only half a user's project
ever runs, since neither bundler type-checks user code.
...
- **A slot-backed tool gets SHORTER in its own file.** A standalone tool file
  ...
  belongs to the SLOT — so `text-adventure-agent`'s eight tools are
...
- **Module state shared by two tools needs a module** —
  `medication-safety-agent`'s `fda.ts` says so in place, and `konsistent.json`'s
  `template-tool-owns-no-sibling` is why reaching it through the other tool FILE
  is the version that goes wrong quietly.
- **A workflow DECLARATION needs a home that is neither half.**
  `research-handoff-agent` and `meeting-recap-agent` reach a run by passing the
  definition rather than its name (which is what types the input), and four or
  five tool files each name it, so `workflow({ … })` moved from `agent.ts` into
  `shared.ts`. The BODY stays in `workflows/` by convention, so a spec can
  import the steps without the agent.
...
`retail-orders-agent`'s `registry.test.ts` is the one template that still writes
the lowering out, with **`deployedAgent`** (`@alexkroman1/aai/testing`) over its
own glob and a `?raw` read of `system-prompt.md`. That is deliberate: the tool
registry is that file's subject, and it is the worked example for a project
whose runner cannot register a Vite plugin. **`withDiscoveredTools`, the
tools-only half this section used to name, is not exported** — a spec that
lowered the tools and forgot the prompt measured the framework default and
...
from a seedless `emptyRetailState()` rather than from the projection, because
