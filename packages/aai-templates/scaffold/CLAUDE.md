...
- No SDK imports needed for agent code -- tools and hooks are plain TypeScript
  files
- Custom UI goes in `client.tsx` alongside `agent.json` -- **uses React** (import
  hooks from `react`, not `preact/hooks`)
...
my-agent/
  ...
  client.tsx          # Custom UI (optional, uses React)
...
Add `client.tsx` alongside `agent.json`. Call `defineClient()` to set up the
UI. There are three usage tiers:

**Tier 1 -- config only** (no custom component, uses default UI):

```tsx
import "@alexkroman1/aai-ui/styles.css";
import { defineClient } from "@alexkroman1/aai-ui";

defineClient({ title: "My Agent" });
```

**Tier 2 -- config + sidebar** (default chat with a custom sidebar):

```tsx
import "@alexkroman1/aai-ui/styles.css";
import { defineClient, useToolResult } from "@alexkroman1/aai-ui";
import { useState } from "react";

function MySidebar() {
  const [items, setItems] = useState<string[]>([]);
  useToolResult("add_item", (result: any) => setItems((p) => [...p, result.item]));
  return <div className="p-4">{items.map((it, i) => <p key={i}>{it}</p>)}</div>;
}

defineClient({ title: "My Agent", sidebar: MySidebar });
```

**Tier 3 -- full custom component**:
...
function MyApp() {
  const { state, messages, userTranscript, started, running, start, toggle, reset } = useSession();
  return (
    <div>
...
defineClient({ component: MyApp, theme: { primary: "#e94560" } });
```

**Rules:**

- Always import `"@alexkroman1/aai-ui/styles.css"` at the top -- without it,
  default styles won't load
- Call `defineClient(config)` at the end of the file
- Use `.tsx` file extension for JSX syntax
- Import hooks from `react` (`useEffect`, `useRef`, `useState`, etc.)
- Tailwind for layout (`className="flex gap-4"`), `useTheme()` for colors
...
```ts
defineClient({
  title?: string;        // Shown in header and start screen
  component?: FC;        // Custom root component (replaces default UI)
  sidebar?: FC;          // Custom sidebar component (used with default chat)
  ...
  target?: string;       // CSS selector or DOM element (default: "#app")
  platformUrl?: string;  // Server URL (auto-derived from location.href)
});
```
...
Import from `@alexkroman1/aai-ui`:
...
- `useSession()` -- returns session state and control methods (see below).
- `useTheme()` -- returns `{ bg, primary, text, surface, border }` CSS values.
- `useToolResult(callback)` -- fires once per completed tool call.
- `useToolCallStart(callback)` -- fires when a tool call begins (status "pending").

### Session API (`useSession()`)

`useSession()` returns a plain object (React state under the hood) with
reactive fields and control methods. Destructure what you need:

```ts
const { state, messages, toolCalls, userTranscript, agentTranscript, error, started, running, start, toggle, reset, cancel, disconnect } = useSession();
```

| Field             | Type                   | Description                                                  |
| ----------------- | ---------------------- | ------------------------------------------------------------ |
| `state`           | `AgentState`           | "disconnected", "connecting", "ready", "listening", etc.     |
| `messages`        | `Message[]`            | `{ role, content }` objects                                  |
| `toolCalls`       | `ToolCallInfo[]`       | `{ callId, name, args, status, result? }` -- tool calls      |
| `userTranscript`  | `string \| null`       | `null` = not speaking, `""` = speech detected, string = text |
| `agentTranscript` | `string \| null`       | `null` = not speaking, string = streaming agent response     |
| `error`           | `SessionError \| null` | `{ code, message }`                                          |
| `started`         | `boolean`              | Whether session has been started                             |
| `running`         | `boolean`              | Whether session is active                                    |

**Methods:** `start()`, `toggle()`, `reset()`, `disconnect()`, `cancel()`

Fields are plain React state values -- no `.value` needed. The component
...
| `useToolCallStart((toolName, args, tc) => {})`      | Fires when a tool call begins (status "pending").                                            |
| `useTheme()`                                        | Returns `{ bg, primary, text, surface, border }` from the active theme.                      |
...
Tool returns object on server
  ...
  -> session.toolCalls state updates -> component re-renders
...
when the `toolCalls` array updates for unrelated reasons.

**What about multiple calls to the same tool?** Each call has a unique
`callId`. If the agent calls `get_recipe` three times, your callback
fires three times -- once for each call -- with the individual result. Use
`toolCall.callId` if you need to distinguish them, or `toolCall.args`
...
```ts
type ToolCallInfo = {
  callId: string;                // Unique ID -- used for deduplication
...
function RecipeAgent() {
  ...
  return (
    <div className="flex gap-4 h-full">
      ...
      {recipe && (
        <div className="w-80 p-4 bg-aai-surface rounded-lg">
          <h2 className="text-aai-text font-bold text-lg">{recipe.name}</h2>
          <h3 className="text-aai-text-muted mt-2 font-semibold">Ingredients</h3>
          <ul className="list-disc ml-4 text-aai-text text-sm">
            {recipe.ingredients.map((ing) => <li key={ing}>{ing}</li>)}
          </ul>
          <h3 className="text-aai-text-muted mt-2 font-semibold">Steps</h3>
          <ol className="list-decimal ml-4 text-aai-text text-sm">
...
defineClient({ component: RecipeAgent });
```
...
**Do NOT use `useEffect` + `toolCalls` to build derived state.**
That pattern re-processes every tool call on every render, causing
duplicates (e.g. items added to the cart multiple times). `useToolResult`
handles deduplication correctly.

```tsx
// BAD -- duplicates on every re-render
...
- `userTranscript`: `null` = user is not speaking, `""` = speech detected but
  no text yet (show "..."), non-empty string = partial/final transcript
- `agentTranscript`: `null` = agent is not speaking, non-empty string =
...
function App() {
  const { started, start, messages, toolCalls } = useSession();
  if (!started) return <button onClick={start}>Start</button>;
      ...
      {messages.map((m, i) => (
        <div key={i}>
          ...
          {toolCalls
            ...
            .map((tc) => <ToolCallBlock key={tc.callId} toolCall={tc} />)}
        </div>
      ))}
    </div>
  );
...
function ShopAgent() {
  ...
  const sidebar = (
    <div className="p-4">
      <h3 className="text-aai-text font-bold">Cart ({cart.length})</h3>
      {cart.map((i) => <p key={i.id} className="text-aai-text text-sm">{i.name} -- ${i.price}</p>)}
    </div>
  );
...
defineClient({ component: ShopAgent });
```
...
function MyCard() {
  ...
  return (
    <div className="flex items-center gap-2 p-4 rounded-aai"
...
For static layout and spacing, use Tailwind classes. For dynamic color values,
use inline styles with `useTheme()`. For keyframes and media queries, use
injected `<style>` tags:
...
**Destructure session fields at the top of your component:**

```tsx
function MyComponent() {
  const { state, messages, userTranscript, started, start } = useSession();
  // Use state, messages, etc. as plain values throughout the render
}
```

**Auto-scrolling messages** -- `MessageList` and `ChatView` include auto-scroll
built-in. For a fully custom message list, use a `useEffect` with a sentinel ref:
...
function MyChat() {
  const { messages, userTranscript } = useSession();
...
- **Using `useEffect` to build state from tool calls** -- Use `useToolResult`
  ...
  `toolCalls` in `useEffect` causes duplicates.
