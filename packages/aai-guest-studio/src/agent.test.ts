...
// The guest toolchain, which stays in `aai-guest`: the guest image's Docker
// build context is that package, and it COPYs `toolchain/package.json` and
// `dist/harness.mjs` from the one context. This package resolves it at RUN
// time by searching upward (`toolchainRoot()`), so the reach is test-only.
const TOOLCHAIN_DIR = path.join(import.meta.dirname, "..", "..", "aai-guest", "toolchain");
...
describe("the studio coding agent's definition", () => {
  ...
  test("a registry rebuilt for a second session never serves the first one's tools", async () => {
    ...
    expect(await glob(second, "agent.ts")).toBe("No files found");
    expect(await glob(first, "agent.ts")).toBe("agent.ts");
