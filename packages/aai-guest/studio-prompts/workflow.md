<!--
  ...
  Source: packages/aai-studio-server/src/prompts/ (studioSystemPrompt("workflow"))
...
The user chose **Workflow** when creating this project, so build a STATIC
...
- Nothing here is spoken: a workflow app has no session and no model turn,
...
A workflow app's client.tsx is its FRONT DOOR, not a decoration — the page
...
[User] What should the form ask for?
