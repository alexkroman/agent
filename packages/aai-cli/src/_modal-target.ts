 ...
 *
 * ## The entry IS shared with Deno's now
 *
 * It was not, on the reading that the two read the port from different
 * runtimes and a shared entry would be a template with a hole where that goes.
 * `RUNTIME_PORT_SOURCE` reads both runtimes, so the hole closed — and the
 * copy cost more than duplication: it is what made each artifact runnable only
 * under the runtime whose host emitted it. See `_target-entry.ts`.
 *
 * ## The drain is SHARED, and Modal supplies its other half
 *
 * The `SIGINT`/`SIGTERM` body is `TARGET_DRAIN_SOURCE`, the same text the
 ...
 */
export const MODAL_ENTRY_SOURCE = longLivedEntrySource({
  target: "modal",
  hostNote: [
  ...
  defaultPort: MODAL_PORT,
  drainNote: "app.py forwards the container's stop signal here and waits for this process.",
});
