...
import { loadAgentDef, resolveServerEnv } from "./_server_common.ts";
...
describe("loadAgentDef", () => {
  ...
  test("returns agent def when using defineAgent", async () => {
    await withTempDir(async (dir) => {
      // Must use defineAgent — plain objects get double-wrapped by tsImport
      await fs.writeFile(
        path.join(dir, "agent.ts"),
        [
          `import { defineAgent } from "${path.resolve("sdk/mod.ts")}";`,
