...
/**
 * End-to-end browser tests (Playwright): UI render, WebSocket, conversation
 * flow against a built client served by a static server.
 *
 * Template pack + build workflows live in e2e.test.ts.
 *
...
describe.skipIf(!hasPlaywrightBrowser())("browser: dev server", () => {
  ...
  beforeAll(async () => {
    // Build CLI + start mock npm registry (each e2e suite does its own setup)
    aaiBin = buildCli();
