...
Audited 2026-08 against their docs; most of this surface is exactly what they
recommend. The three places we diverge — `postgres_changes` instead of Broadcast,
deny-all RLS over a grant that already excludes everyone, and per-app Postgres
roles instead of RLS — are each a decision rather than an oversight, and they are
documented with the migrations they constrain: see **`supabase/CLAUDE.md`**. Two
operational facts live there too (the IPv4/direct-connection requirement, and the
legacy-key deprecation). Moved out when this guide hit the 120,000-character cap;
nothing was cut.
...
### Waking a workflow run that has come due — `workflow-wake.ts`

The SDK recovers a released `ctx.sleep` in `runDue()`, which runs **at boot** —
so without a sweep a six-hour sleep resumed whenever somebody next phoned the
agent, and durable meant NOT LOST rather than RUNS ON TIME. The sweep brokers
agents whose journal has work; the boot IS the action. Its module doc carries
the argument (why not pg_cron, why not ask the guest, why the candidate set must
be `aai_platform.agents` — hence `AgentRows.listSlugs`). Two facts belong here:

- **It is the one place the platform reads inside a tenant schema**, and only
  `aai_workflow_runs`/`aai_workflow_blobs`, which the SDK's own store creates.
- **Every replica sweeps with no lock**, because brokering is idempotent and a
  guest's fleet-wide identity is its Modal NAME — the reasoning that deleted
  `sandbox_registry`. `WORKFLOW_WAKE_POLL_MS` is the lateness an author sees.
