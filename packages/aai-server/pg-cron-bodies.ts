...
// The prefix the blob GC's uploads arm matches on. It owes one rule beyond the
// LIKE-wildcard one above: it must be a SINGLE path segment, because that arm
// reads the slug and the id back out of a key by position
// (`split_part(name, '/', 2)` and `3`). A nested prefix would shift both and the
// arm would join on the wrong halves of somebody's key.
