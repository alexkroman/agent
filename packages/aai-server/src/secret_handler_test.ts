...
test("secret list returns var names for deployed agent", async () => {
  ...
  expect((await res.json()).vars).toEqual(["ASSEMBLYAI_API_KEY"]);
});
...
test("secret set merges new vars", async () => {
  ...
  expect(setBody.keys.sort()).toEqual(["ASSEMBLYAI_API_KEY", "MY_KEY"]);
});
...
test("secret delete removes a key", async () => {
  ...
  expect((await listRes.json()).vars).toEqual(["ASSEMBLYAI_API_KEY"]);
});
