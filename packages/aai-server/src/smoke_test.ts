...
import { z } from "zod";
// direct_executor.ts has no package export, so use relative path through workspace
import { buildAgentConfig } from "../../aai/direct_executor.ts";
import { createTestOrchestrator } from "./_test_utils.ts";
...
describe("cross-package smoke: SDK → server deploy", () => {
  ...
  test("deployed agent is accessible via health and page endpoints", async () => {
    ...
    expect((await healthRes.json()).slug).toBe("accessible-agent");
