...
/** This package's `src/`, which every path here is relative to. */
const SRC = path.join(import.meta.dirname, "..");

const source = (file: string): string => readFileSync(path.join(SRC, file), "utf-8");
 ...
 *
 * **It walks RECURSIVELY from `src/`, and that is not a refinement.** It read one
 * directory, which was the whole package until `platform/`, `sandbox/`, `modal/`,
 * `microsandbox/` and `guest/` existed — and this file moved into the last of
 * them, so the unchanged version would have scanned 20 files and tripped the
 * floor. A floor turns that into a failure; without one it is a rule that
 * silently stopped covering 85% of what it names.
 */
function packageSources(): string[] {
  ...
  walk("");
  if (out.length < 120) {
    throw new Error(`only ${out.length} sources found under ${SRC}`);
  }
  return out;
}
