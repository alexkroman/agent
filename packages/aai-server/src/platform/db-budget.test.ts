...
import { fleetMaxContainers, platformDbBudget } from "./db-capacity.ts";
import { platformDbConnectionsPerReplica, QUEUE_NOTIFY_LISTEN } from "./db-limits.ts";
...
describe("platform database connection budget", () => {
  ...
  test("the connection string is direct, not a transaction-mode pooler", () => {
    ...
    const source = readFileSync(path.join(import.meta.dirname, "lock.ts"), "utf-8");
