...
const SWEEPERS = [
  "packages/aai-server/store-conformance.scenario.test.ts",
  "packages/aai-studio-server/studio-store-conformance.scenario.test.ts",
];
...
describe("a conformance sweep is scoped to its own process", () => {
  ...
  test("the studio package spells the SAME grammar, or the pids stop separating", () => {
    ...
    const studio = readFileSync(
      join(ROOT, "packages/aai-studio-server/studio-store-conformance.ts"),
      "utf8",
    );
