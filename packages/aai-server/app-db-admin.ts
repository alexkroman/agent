 ...
 * with its own pool and its own project: a fleet is several Supabase projects,
 * and the ref is per cluster, so each one's channel is resolved from its own URL.
 *
...
export function extraAppDbTargets(env: NodeJS.ProcessEnv): AppDbTarget[] {
  return (env.APP_DB_URLS ?? "")
    ...
    .map((url) => {
      ...
      return { url, sql, admin } satisfies AppDbTarget;
    });
}
