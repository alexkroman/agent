...
describe("isPrivateIp: comprehensive private range coverage", () => {
  ...
  ])("blocks all RFC 1918 private ranges: isPrivateIp(%s) === %s", (ip, expected) => {
  ...
  ])("blocks carrier-grade NAT range (100.64.0.0/10): isPrivateIp(%s) === %s", (ip, expected) => {
    expect(isPrivateIp(ip)).toBe(expected);
  });
  ...
  test.each([
    ...
    ["127.0.0.0", true],
  ])("blocks loopback range (127.0.0.0/8): isPrivateIp(%s) === %s", (ip, expected) => {
    expect(isPrivateIp(ip)).toBe(expected);
  });
  ...
  ])("blocks link-local range (169.254.0.0/16): isPrivateIp(%s) === %s", (ip, expected) => {
    expect(isPrivateIp(ip)).toBe(expected);
  });
  ...
  ])("blocks benchmarking range (198.18.0.0/15): isPrivateIp(%s) === %s", (ip, expected) => {
    expect(isPrivateIp(ip)).toBe(expected);
  });
  ...
  ])("blocks IANA special-purpose (192.0.0.0/24): isPrivateIp(%s) === %s", (ip, expected) => {
    expect(isPrivateIp(ip)).toBe(expected);
  });
  ...
  ])("blocks multicast and reserved ranges: isPrivateIp(%s) === %s", (ip, expected) => {
    expect(isPrivateIp(ip)).toBe(expected);
  });
  ...
  ])("correctly identifies public IPs: isPrivateIp(%s) === %s", (ip, expected) => {
    expect(isPrivateIp(ip)).toBe(expected);
  });
});
