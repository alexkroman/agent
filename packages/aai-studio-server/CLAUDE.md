---
summary: >-
  Browser studio: workspaces, coding agent, previews, Publish, LLM selection,
  studio evals, the two-package/one-deployment composition
read_when: >-
...
**Moved to [`SSE-CLAUDE.md`](SSE-CLAUDE.md)** beside this file: the studio's two
event streams are the only long-lived responses the combined deployment serves,
and everything a long-lived response needs — shutdown, timeouts, heartbeats —
is argued there. Read it before touching either stream or the lifecycle under
them.
...
- **A long-lived connection is ONE Modal input, so the function `timeout`
  ...
  was rejected, is in [`SSE-CLAUDE.md`](SSE-CLAUDE.md).
