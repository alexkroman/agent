...
- 49daf83: The browser session's audio path — the mic grant, the worklets, and the `VoiceIO` they produce — is a statechart (`session-core-audio-state.ts`) rather than a latch, an epoch counter and two buffers spread across `ConnState`.
