...
import { createMockSignals, flush, installMockWebSocket, setupSignalsEnv } from "./_test_utils.ts";
...
describe("UI integration: signals → component rendering", () => {
  ...
  test("start → connect → events flow through to signals", async () => {
    ...
    expect(env.session.messages.value).toEqual([{ role: "user", text: "hello world" }]);
