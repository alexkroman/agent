...
- `components/` — UI components (console-shell, chat-view, controls,
  ...
  button, aai-logo, tool-config-context), the chrome kit (session-state-dot,
  session-controls, conversation-view) and the two workflow-page pieces
...
**`<ConversationView>` (`components/conversation-view.tsx`) is the STRUCTURE
over that data, with every bubble a render slot** — the pinned `AutoScroll`, the
empty-state guard, the interleave with its prefixed keys (`m<id>` / `t<callId>`,
so a message id and a tool call spelled with the same digits cannot share one),
the streaming row (by default `renderMessage` over a synthetic assistant message
with `id: -1`), the announced `role="status"` thinking row, and the transcript —
inline as the last row, or `transcriptPosition="below"` for the strip a two-pane
board pins under the scroll. The three chromes had each written those fifty
lines around the hook, comment about screen readers hearing punctuation
included. `<MessageList>` is now this with the stock bubbles filled in
...
**What the three chromes DID take, once the frame was cut finer.** The frame
stayed theirs; what moved is the three pieces a header, a footer and a
conversation column are made of. `<SessionStateDot colors={…}>` is the live dot
and word on its own `useSessionStatus()` subscription: the `satisfies
Record<AgentState, string>` palette is the PROP, which is what keeps the shared
part — the exhaustive lookup, the `AGENT_STATE_LABELS` fallback under a partial
`labels`, the listening/thinking pulse — from taking the palette hostage (the
argument `agent-state-labels.ts` used to make against any dot component), and
`ConsoleShell`'s own dot is the same `StateDot` coloured from the theme.
`<SessionControls>` is the FULL row `Controls` never was — Start before the
call, then Pause/Resume, New Conversation, End — deciding which buttons exist
and what each presses (the `end(); start()`-not-`reset()` argument two chromes
carried verbatim lives on it once) while `renderButton` decides the look;
`useSessionControls()` under it is the two one-field subscriptions and four
methods every row opened with, published for the chrome whose buttons are too
unusual even for the slot — no template is that chrome yet. And `.aai-scroll`
...
terminal, the completed body as a function of the TYPED output, and
`<WorkflowRunError>` last. Its spec asserts the composition the way
`console-shell.test.tsx` asserts its banner.
...
Moved whole to [`PLAYBACK-CLAUDE.md`](PLAYBACK-CLAUDE.md), a sibling of this
guide of the kind `AGENTS.md` describes: the measurements
`worklets/playback-tuning.test.ts` produced for `PLAYBACK_JITTER_MS` and
`PLAYBACK_REFILL_MS`, why the jitter constant is redundant BY CONSTRUCTION, what
the pacer costs playback, and what was deliberately left unchanged. Reference,
read on demand — this guide had three hundred characters left under its cap.
