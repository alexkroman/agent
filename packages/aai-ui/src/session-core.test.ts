...
describe("createBrowserSession", () => {
  ...
  describe("binary audio handling", () => {
    ...
    it("audio chunk ignored in error state with error set", () => {
      ...
      // `fatalError` latches it (see "A FATAL error survives the frames that
      // follow it" in `packages/aai-ui/src/CLAUDE.md`), so deleting the guard
      // flips the state to "speaking" while leaving the banner up — a dead session reading as live, which is
