 ...
 * `packages/aai-gates/src/gate-wiring.test.ts` is the guard that CANNOT be
...
// The three turbo-invocation constants, with their docs — see that module for
// why the GATES table stayed here and they did not.
import { SECOND_TURBO, TURBO_TASKS } from "./_check-turbo-tasks.mjs";
...
const GATES = [
  ...
  {
    script: "check:module-tests",
    phase: "ratchets",
    fatal: false,
    why: "check:coverage-per-file can be satisfied INCIDENTALLY — a module dragged over the 50% floor by a test written for something that imports it. Four aai-guest modules had no test file at all while sitting at 85.71%, 100% and 100%, so nothing in the repo noticed that the studio agent's whole tool registry had no assertion of its own. Pure fs, per-file allowlist, goal zero.",
  },
  {
