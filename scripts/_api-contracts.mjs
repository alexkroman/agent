...
/**
 * For each capability of a package, the names its SIBLINGS contract.
 *
 * Read off the entry points rather than the committed epochs, so it says what
 * the tree claims today and cannot be circular: an epoch's hash would
 * otherwise depend on the epoch files a run is about to rewrite.
 *
 * Always computed over EVERY capability of the package, whatever subset is
 * being extracted — see the `foreign` parameter of {@link hashableBody}.
 */
function foreignNamesByCapability(pkg) {
  const owned = capabilities(pkg).map((capability) => ({
    capability,
    names: parseEntrypoint(pkg, capability).names,
  }));
  return new Map(
    owned.map(({ capability, names: own }) => {
      const foreign = new Set();
      for (const other of owned) {
        if (other.capability === capability) continue;
        for (const name of other.names) if (!own.has(name)) foreign.add(name);
      }
      return [capability, foreign];
    }),
  );
}
 ...
 * body: `_api-contracts-hash.mjs` normalizes away the differences that cannot
 * break a consumer, and carries the measurements that say why. API Extractor's
 * preamble ("Do not edit this file…") is the original instance — identical in
 * every report and the tool's, not ours, so hashing it would make an
 * api-extractor upgrade that reworded one line bump every epoch at once, each
 * demanding a classification for a change to nothing. eve hashes the whole file
 * and pays that; there is no reason to copy it.
 *
...
export function generateCapabilityReports(pkg, names = capabilities(pkg)) {
  ...
  try {
    ...
    const foreignNames = foreignNamesByCapability(pkg);
    ...
    for (const { capability, config } of configs) {
      ...
      reports.set(capability, {
        ...
        exports: collectExports(text, label)
          ...
          .sort(),
        sha256: createHash("sha256").update(hashable).digest("hex"),
      });
    }
    return reports;
  } finally {
