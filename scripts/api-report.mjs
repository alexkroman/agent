...
if (!CHECK) {
  ...
  console.log(
    `api-report: wrote ${reportCount} report(s) under packages/*/etc/, ` +
      `${COMBINED_FILE}, ${EXPORTS_FILE} and ${INDEX_FILE}.`,
  );
  process.exit(0);
}
...
console.log(
  `api-report: ${reportCount} API report(s), ${COMBINED_FILE}, ${EXPORTS_FILE} and ` +
    `${INDEX_FILE} up to date. ✓`,
);
